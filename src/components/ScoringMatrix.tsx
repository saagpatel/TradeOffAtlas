...
export function ScoringMatrix() {
	...
	function handleDeleteOption(id: number, name: string) {
		if (window.confirm(`Delete option "${name}"? This cannot be undone.`)) {
			removeOption(id).catch((err: unknown) => {
	...
	function handleDeleteCriterion(id: number, name: string) {
		if (
			...
			removeCriterion(id).catch((err: unknown) => {
			...
			<table className="border-collapse text-sm text-slate-200 w-full">
				<thead>
					<tr className="border-b border-slate-700">
							...
							>
								<div className="flex flex-col items-center gap-1">
									<div className="flex items-center gap-1 w-full justify-center">
										<InlineEdit
											value={c.name}
											onCommit={(name) => updateCriterionField(c.id, { name })}
									...
									<div className="flex items-center gap-1 text-xs text-slate-500">
										...
										<input
											...
											onBlur={(e) => {
												...
												updateCriterionField(c.id, { weight: clamped });
											}}
											onKeyDown={(e) => {
												if (e.key === "Enter") {
						...
						{/* Add criterion button in header */}
						...
						>
							...
							<td className="px-4 py-2">
								<div className="flex items-center gap-1 min-w-0">
									<InlineEdit
										value={opt.name}
										onCommit={(name) => updateOptionField(opt.id, { name })}
										placeholder="Option"
							...
							{criteria.map((c) => (
								<td key={c.id} className="px-3 py-2 text-center">
									<ScoreCell
										...
										onChange={(score) => setScore(opt.id, c.id, score)}
									/>
								</td>
							))}

							{/* Empty cell under the + criterion button */}
							<td />
